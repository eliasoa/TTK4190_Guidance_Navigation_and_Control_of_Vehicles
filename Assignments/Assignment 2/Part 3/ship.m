function [xdot,u] = ship(x,u,nu_c,tau_ext,h)
...
thr     = rho * Dia^4 * KT * abs(n) * n;    % thrust (N)
Q       = rho * Dia^5 * KQ * abs(n) * n;    % torque (N)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Thing to fix integration of main engine torque dynamics
persistent Qm 
    if isempty(Qm)
        Qm = 0;
    end

Qd      = rho*Dia^5*KQ*abs(n_c)*n_c;        % desired torque (N)
Qm_dot  = (Qd - Qm)/Tm;                     % torque dynamics

n_dot   = (1/Im) * (Qm - Q);                % shaft speed dynamics
Qm      = euler2(Qm_dot,Qm,h);              % Euler integration
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
